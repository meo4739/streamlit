...
df = pd.read_csv('streamlit/data_subway_in_seoul.csv', encoding = 'CP949') # encoding='cp949'  읽어오고 확인하기 
st.dataframe(df)

st.text("2. 구분이 '하차'인 행만 새로운 데이터프레임으로 저장 & 확인") 
df_off = df.loc[df['구분']=='하차'] # 구분이 '하차'인 행만 저장
st.dataframe(df_off)
...
df_line_groupby = df_line_melted.groupby(['호선','시간'])['인원수'].sum().reset_index()
