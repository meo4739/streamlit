...
if st.button('data copyright link'): #  button을 누르면 원본데이터 주소가 나타남: https://www.data.go.kr/data/15044247/fileData.do
    st.write('https://www.data.go.kr/data/15044247/fileData.do')

if st.checkbox('원본 데이터 보기'): #  checkbox를 선택하면 원본 데이터프레임이 나타남
...
df_line_groupby = df_line_melted.groupby(['호선','시간'])['인원수'].sum().reset_index() # '호선','시간' 별 '인원수' 합,  as_index=False 저장 & 확인 
if st.checkbox('호선, 시간대별 인원 집계 데이터 보기'): 
    st.subheader('5. 호선, 시간대별 인원 집계 데이터- df_line_groupby')
    st.dataframe(df_line_groupby)

st.subheader('전체 호선 시간대별 하차 인원 (5.df_line_groupby)') # altair mark_line 차트 그리기
